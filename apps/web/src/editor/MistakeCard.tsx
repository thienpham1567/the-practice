...
export function MistakeCard({
  ...
  pick: { mark: WritingMark; x: number; y: number; containerWidth: number };
}) {
  // Gạch nằm sát đỉnh khung thì bung lên trên sẽ bị toolbar che — lật xuống.
  const flipBelow = shouldFlipBelow(pick.y);
  // Gạch sát mép trái/phải thì thẻ canh giữa sẽ tràn ra ngoài — kéo vào trong.
  const left = clampAnchorX(pick.x, pick.containerWidth, CARD_MAX_WIDTH_PX);
